...
def convert_subgraph_to_module(graph, full_graph, num_subgraphs, module_name, initialize_weights,
                               model_template_filename, output_filename, arch):
    ...
    # added by keke
    import_statements.append("import torch.nn as nn")
    for node in nodes:
        layer_call = None
        ...
        if (node.node_desc.startswith('Minkowski')):
            ...
            #layer_declaration = layer_declaration.replace("out=", "out_channels=")
        else:
            layer_declaration = node.node_desc
        print("layer_declaration:", layer_declaration)
        ...
        if len(in_edges) == 0 and node.node_desc.startswith("Input"):
            ...
            if node.node_desc.startswith("Size"):
                ...
                m = re.findall(r'-?\d+', node.node_desc)
                if m is None:
                    ...
                    # added by kk
                    if len(size_node_ids) == 0:
                        layer_call = "%s = %s.view(%s)" % (output_name,
                                                               output_names[input_node_id],
                                                               ", ".join(m))
                    else:
                        size = m[1]
                        layer_call = "%s = %s.view(%s, %s)" % (output_name,
                                                               output_names[input_node_id],
                ...
                else: #changed by keke for Minkowski
                    layer_call = "%s = self.cat(%s)" % (
                        output_name,
                        ", ".join([output_names[in_node.node_id]
...
if __name__ == '__main__':
    ...
    # changed by keke
    initialize_weights = (args.arch == "vgg16" or args.arch == "resnet50" or args.arch == "minkunet")
    ...
    for i, subgraph in enumerate(subgraphs):
        ...
        num_inputs, num_outputs = convert_subgraph_to_module(subgraph, full_graph, len(subgraphs),
                                                             ...
                                                             os.path.join(args.output_directory,
                                                                          module_filename),
                                                             args.arch) #added by keke for mink
