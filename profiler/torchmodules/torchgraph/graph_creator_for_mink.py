...
class SparseTensorWrapper(object):
    ...
    @property
    def F(self):
        #print("self.tensor:", self.tensor)
        return self.tensor.F

    @property
    def coords_key(self):
        return self.tensor.coords_key

    @property
    def coords_man(self):
        return self.tensor.coords_man
    ...
    def size(self, dim=None):
        if dim is None:
            ...
            dim_str = "(%d)" % dim
        wrapped_result = SparseTensorWrapper(result, "Size%s" % dim_str, self.graph_creator,
                                       activation_size=4)
    ...
    def view(self, *wrapped_sizes):
        ...
        for wrapped_size in wrapped_sizes:
            if isinstance(wrapped_size, SparseTensorWrapper):
        ...
        if len(sizes) == 1:
            wrapped_result = SparseTensorWrapper(result, "View", self.graph_creator,
                                           activation_size=self.node().activation_size)
        else:
            wrapped_result = SparseTensorWrapper(result,
    ...
    def __add__(self, other):
        ...
        if isinstance(other, SparseTensorWrapper):
        ...
        wrapped_result = SparseTensorWrapper(result_tensor, "Add", self.graph_creator,
    ...
    def __iadd__(self, other):
        ...
        other_activation_size = other.node().activation_size
        assert(self_activation_size == other_activation_size)
        wrapped_result = SparseTensorWrapper(self.tensor, "Add(inplace)", self.graph_creator,
                                       activation_size=self_activation_size)
    ...
    def __mul__(self, other):
        ...
        wrapped_result = SparseTensorWrapper(result, "Mul", self.graph_creator)
        self.graph_creator.graph.add_edge(self._node, wrapped_result.node())
        self.graph_creator.graph.add_edge(other.node(), wrapped_result.node())
        return wrapped_result
    ...
    def __getitem__(self, key):
        ...
        try:
            ...
            activation_size = self.node().activation_size
        wrapped_result = SparseTensorWrapper(result_tensor, "__getitem__(%s)" % key, self.graph_creator,
                                       activation_size=activation_size)
        self.graph_creator.graph.add_edge(self._node, wrapped_result.node())
        return wrapped_result
    ...
    def transpose(self, *args):
        ...
        wrapped_result = SparseTensorWrapper(result_tensor, "Transpose(%s)" % args_str,
                                       self.graph_creator,
                                       activation_size=self.node().activation_size)
        self.graph_creator.graph.add_edge(self._node, wrapped_result.node())
        return wrapped_result
...
def cat(wrapped_tensors, dim):
    ...
    for wrapped_tensor in wrapped_tensors:
        if isinstance(wrapped_tensor, SparseTensorWrapper):
    ...
    wrapped_result = SparseTensorWrapper(result, "Concat(%d)" % dim, graph_creator,
                                   activation_size=activation_sizes[0])
    for wrapped_tensor in wrapped_tensors:
        if not isinstance(wrapped_tensor, SparseTensorWrapper):
            wrapped_tensor = SparseTensorWrapper(wrapped_tensor, "Input", graph_creator)
        graph_creator.graph.add_edge(wrapped_tensor.node(), wrapped_result.node())
    return wrapped_result


class GraphCreatorForMink(object):
    def __init__(self, model, summary, module_whitelist):
        ...
        self.summary = copy.copy(summary) #copy.deepcopy(summary)
        self.forward_original_methods = {}
        self.graph = graph.Graph()
        self.inputs = {}
        #print("Copied summary:", self.summary)
    ...
    def hook_modules(self, module, root=False):
        ...
        def forward_wrapper(self, *wrapped_inputs):
            ...
            for i in range(len(wrapped_inputs_list)):
                if isinstance(wrapped_inputs_list[i], SparseTensorWrapper):
                    ...
                    if key in this_creator.inputs:
                        ...
                        wrapped_inputs_list[i] = SparseTensorWrapper(wrapped_inputs_list[i],
            ...
            wrapped_result = SparseTensorWrapper(result, str(self), this_creator)
        ...
        for name, sub_module in sub_modules.items():
            ...
            if len(sub_sub_modules) == 0 or sub_module_name in self.module_whitelist:
                ...
                if sub_module not in this_creator.forward_original_methods:
                    #print("update forward_original_methods :", sub_module_name)
            ...
            if len(sub_sub_modules) > 0 and sub_module_name not in self.module_whitelist:
                ...
                #
                #print("hook_module :", sub_module_name)
